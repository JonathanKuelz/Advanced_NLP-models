...
class Vocabulary:
    def __init__(self):
        self.words = {"-": 0, "?": 1, "<UNK>": 2, ".": 3}  # Keep 3 as point to mark end of sentence
        self.inv_words = {0: "-", 1: "?", 2: "<UNK>", 3: "."}
        self.next_id = 4
        self.punctations = [".", "?", ","]
    ...
    @classmethod
    def from_dict(cls, word2id):
        if '-' not in word2id:
            id2word = {v+1: k for k, v in word2id.items()}  # Make space for the empty '-' word at index 0
            id2word[0] = '-'
        else:
            id2word = {v: k for k, v in word2id.items()}
        res = cls()
        res.words = {v: k for k, v in id2word.items()}
        res.inv_words = id2word
        res.next_id = len(id2word)
        return res
