...
class bAbiDataset(NLPTask):
    ...
    def _load_task(self, filename, task_name):
        ...
        with open(filename, "r") as f:
            for line in f:
                ...
                currTask.append(
                    Sentence(self.vocabulary.sentence_to_indices(line[0]),
    ...
    def verify_result(self, test, data, net_output):
        ...
        mask = 1.0 - ref.eq(0).float()  # 1 if output calculated, 0 else
    ...
    def show_test_results(self, x_value, test, x_label='Epoch'):
        ...
        acc_percent = [res[k] * 100 for k in all_keys]

        n_passed = sum([int(p >= 95) for p in acc_percent])
        n_total = len(acc_percent)
        if len(all_keys) > 1:
            # err_precent = err_precent + [sum(err_precent) / len(err_precent)]
            acc_percent = acc_percent + [sum(acc_percent) / len(acc_percent)]
            all_keys += ["mean"]

        for i, k in enumerate(all_keys):
            t += "<font color=\"%s\">%s: <b>%.2f%%</b></font><br>" % (
            "green" if acc_percent[i] >= 95 else "red", k, acc_percent[i])

        t += "<br><b>Total: %d of %d passed.</b>" % (n_passed, n_total)

        self._ensure_test_wins_exists(xlabel=x_label, ylabel='Accuracy', legend=[i.split("_")[0] if i.startswith("qa") else i for i in all_keys])
