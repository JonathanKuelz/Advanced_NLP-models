...
def init_debug(debug, initial):
    """So debug is not boolean true, but also it is not None"""
...
_EPS = 1e-6  # Epsilon for numeric stability


class AllocationManager(torch.nn.Module):
    """
    This part of the code manages the DNCs memory. The accompanying document makes the following statement about allocation:

    Memory allocation works by maintaining usage counters for every cell. Theseare incremented on memory writes and
    optionally decremented on memory reads (de-allocation).When a new cell is allocated, the one with the lowest usage
    counter is chosen.   De-allocation iscontrolled by a gate, which is based on the address distribution of the
    previous read and decreasesthe usage counter of each cell.
    """
    def __init__(self):
    ...
    def update_usages(self, prev_write_distribution, prev_read_distributions, free_gates):
        # Read distributions shape: [batch, n_heads, cell count] e.g. [2, 4, 256]
        # Free gates shape: [batch, n_heads], de-allocate after read or not

        self._init_consts(prev_read_distributions.device)
        # 1 - elementwisemultiply free_gates by prev_read_distributions
        ...
        # If memory usage counter doesn't exist
...
class WriteHead(torch.nn.Module):
    ...
    def __init__(self, dealloc_content=True, disable_content_norm=False, mask_min=0.0, disable_key_masking=False):
        ...
        self.write_content_generator = ContentAddressGenerator(disable_content_norm, mask_min=mask_min,
    ...
    def forward(self, memory, write_content_key, write_beta, erase_vector, write_vector, alloc_gate, write_gate,
        ...
        # This line calls the AllocationManager.__call__ method, inherited by torch.nn.module
        ...
        self.last_write = WriteHead.create_write_archive(write_dist, erase_vector, write_vector,
...
class ReadHead(torch.nn.Module):
    ...
    def forward(self, memory, read_content_keys, read_betas, forward_dist, backward_dist, gates, read_mask=None,
                debug=None):
        content_dist = self.content_addr_generator(memory, read_content_keys, read_betas, mask=read_mask)

        self.read_dist = backward_dist * gates[..., 0:1] + content_dist * gates[..., 1:2] + forward_dist * gates[...,
                                                                                                           2:]
...
class DNC(torch.nn.Module):
    def __init__(self, input_size, output_size, word_length, cell_count, n_read_heads, controller, batch_first=False,
                 clip_controller=20,
        ...
        self.read_head = RawReadHead(n_read_heads, word_length, use_mask=mask,
    ...
    def _step(self, in_data, debug):
        ...
        controls = controls.clamp(-self.clip_controller,
                                  self.clip_controller) if self.clip_controller is not None else controls
        ...
        forward_dist, backward_dist = self.temporal_link(prev_write["write_dist"] if prev_write is not None else None,
                                                         prev_read_dist, debug=dict_get(debug, "temporal_links"))
    ...
    def forward(self, in_data, debug=None, embed=None, full=True):
        # TODO: Explain how the forward pass of the DNC works
        """
        Forward pass for the Differential Nerual Computer. Inpuut gets ...
        Args:
            in_data: Tokenized NLP input of shape batch x seq_len x channels (changes if self.batch_first=True)
            debug: ???
            embed: An embedding method for the input data
            full: If False, only the output for the last timestep will be returned
        ...
        full_out = torch.stack(out_tsteps, dim=1 if self.batch_first else 0)
        if full:
            return full_out
        else:
            return full_out[:, -1, :] if self.batch_first else full_out[-1, :, :]
...
class LSTMController(torch.nn.Module):
    ...
    def init(self, input_size):
        ...
        self.input_sizes = [(self.layer_sizes[i - 1] if i > 0 else 0) + self.layer_sizes[i] + input_size
                            for i in range(len(self.layer_sizes))]
        self.stdevs = [math.sqrt(2.0 / (self.layer_sizes[i] + self.input_sizes[i])) for i in
                       range(len(self.layer_sizes))]
        self.in_to_all = [torch.nn.Linear(input_size, 4 * self.layer_sizes[i]) for i in range(len(self.layer_sizes))]
        self.out_to_all = [torch.nn.Linear(self.layer_sizes[i], 4 * self.layer_sizes[i], bias=False) for i in
                           range(len(self.layer_sizes))]
        self.prev_to_all = [torch.nn.Linear(self.layer_sizes[i - 1], 4 * self.layer_sizes[i], bias=False) for i in
